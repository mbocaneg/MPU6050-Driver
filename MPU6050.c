 ...
 *	mpu6050 typedef to zero. The user provides an mpu6050 instance to
 *	be initialized, along with 2 reference angles(pitch/roll) at which
 *	the mpu6050 is held when calibrating.
 *
 *	params:
 *
 ...
 *
 *	*pitch_reference -- reference pitch measurement at which the mpu6050 is held
 *	during calibration(e.g. 0 for flat, 90 for upright)
 *
 *	*roll_reference -- reference roll measurement at which the mpu6050 is held
 *	during calibration(e.g. 0 for flat, 90 for upright)
 *------------------------------------------------------------------*/
void mpu6050_init(Mpu6050 *mpu6050, double pitch_reference, double roll_reference) {
	...
	mpu6050_calibrate(mpu6050, pitch_reference, roll_reference);
}
 ...
 *	by the variable num_samples) at the set sampling rate. The user provides
 *	2 reference parameters(for pitch and roll, respectively) that serves as
 *	the reference point in which the mpu6050 is held at during calibration. Further
 ...
 *	during calibration(e.g. 0 for flat, 90 for upright)
 *------------------------------------------------------------------*/
void mpu6050_calibrate(Mpu6050 *mpu6050, double pitch_reference, double roll_reference) {
	...
	mpu6050->offset_acc_pitch = (offset_accPitch / (float) num_samples) - pitch_reference;
	mpu6050->offset_acc_roll = (offset_accRoll / (float) num_samples) - roll_reference;
}
